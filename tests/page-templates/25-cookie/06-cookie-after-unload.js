...
describe("e2e/25-cookie/06-cookie-after-unload", function() {
	...
	it("Should have set Unload Time (hd)", function() {
		var cookie = BOOMR.utils.getSubCookies(BOOMR.utils.getCookie("RT"));

		var hd = parseInt(cookie.hd, 36) + parseInt(cookie.ss, 36);

		assert.closeTo(hd, unloadTimeFired, 100);
	});
