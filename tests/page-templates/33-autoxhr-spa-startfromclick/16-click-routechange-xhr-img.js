...
describe("e2e/33-autoxhr-spa-startfromclick/16-click-routechange-xhr-img.js", function() {
	...
	describe("Beacon 2", function() {
		...
		it("Should have Page Load Time ~4070ms", function() {
			assert.closeTo(tf.beacons[1].t_done, t.xhrTimes.xhr1.duration + t.imgTimes.img1.duration + 30 + 30 + 10, 100);
		});

		it("Should have Page Load Time >= 4070ms", function() {
			assert.operator(tf.beacons[1].t_done, ">=", 4070);
		});
		...
		it("Should have Front End Time ~2070ms", function() {
			assert.closeTo(tf.beacons[1].t_page, t.imgTimes.img1.duration + 30 + 30 + 10, 100);
		});
	});
});
