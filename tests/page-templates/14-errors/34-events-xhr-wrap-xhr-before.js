...
describe("e2e/14-errors/34-events-xhr-wrap-xhr-before", function() {
  ...
  it("Should have via = GLOBAL_EXCEPTION_HANDLER on EventTarget supported browsers", function() {
  ...
  it("Should have via = EVENTHANDLER on non-EventTarget supported browsers", function() {
    if (!window.EventTarget) {
      var b = tf.lastBeacon();
      var err = BOOMR.plugins.Errors.decompressErrors(C.jsUrlDecompress(b.err))[0];
  ...
  it("Should have called the wrapped XMLHttpRequest addEventListener once", function() {
