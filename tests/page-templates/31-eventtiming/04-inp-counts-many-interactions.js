...
describe("e2e/31-eventtiming/04-inp-counts-many-interactions", function() {
  ...
  describe("Page Load beacon", function() {
    ...
    it("Should have set Incremental Interaction to Next Paint (et.inp.inc) to be the 4th largest interaction on the Page Loads beacon, based on 500+ interaction, if when we only have 4 long interactions", function() {
      assert.equal(parseInt(tf.beacons[0]["et.inp.inc"], 10), 50);
    });
  ...
  describe("Unload beacon", function() {
    it("Should have set Interaction to Next Paint (et.inp) to be the 4th largest interaction on the Unload beacon, based on 500+ interaction, if when we only have 4 long interactions", function() {
      assert.equal(parseInt(tf.beacons[1]["et.inp"], 10), 50);
    });
