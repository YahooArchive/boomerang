...
describe("e2e/31-eventtiming/03-inp-counts", function() {
  ...
  describe("Page Load beacon", function() {
    ...
    it("Should have set Incremental Interaction to Next Paint (et.inp.inc) to be the 2nd largest interaction on the Page Load beacon, based on 50+ interactions", function() {
      assert.equal(parseInt(tf.beacons[0]["et.inp.inc"], 10), 200);
    });
  ...
  describe("Unload beacon", function() {
    it("Should have set Interaction to Next Paint (et.inp) to be the 2nd largest interaction on the Unload beacon, based on 50+ interactions", function() {
      assert.equal(parseInt(tf.beacons[1]["et.inp"], 10), 200);
    });
